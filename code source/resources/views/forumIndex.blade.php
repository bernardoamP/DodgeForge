...
<div class="container-fluid mt-2">
    ...
    <div class="row cadreblanc mt-1">
        <div class="col-sm-10 d-flex justify-content-center align-items-center">
            <h1 class="mx-2 font-weight-bold text-with-shadow">ESPACE FORUM</h1>
            <div class="d-none d-sm-block mx-3 d-flex align-items-center"><p class="d-flex mb-0 align-items-center font-weight-bold text-with-shadow"><b>Le lieu de toutes les questions!!</b></p></div>
        </div>
        @auth
            <div class="col-sm-2 d-flex align-items-center justify-content-center mb-1 mb-sm-0">
            <button type="button" class="rounded degrade-diagonal-rouge float-sm-end" data-bs-toggle="modal" data-bs-target="#modalFormForum">
            ...
            <div id="searchCollapse" class="collapse d-sm-flex flex-column mt-3 mb-2">
                ...
                <div class="mt-3 mb-2 cadreblanc">
                    ...
                    <div class="dropdown mx-1 justify-content-center align-items-center">
                        <button class="rounded-2 degrade-diagonal-rouge dropdown-toggle w-100 mb-1" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                            @isset($currentUser)
                        ...
                        <ul class="dropdown-menu text-center degrade-diagonal-rouge scroll-autors">
                            <li><a class="dropdown-item text-light" href="{{route("forumPostByUsers", ['id'=>0])}}">Tous</a></li>
                            <div class="dropdown-divider"></div>
                            @foreach ($users as $user)
                                <li><a class="dropdown-item text-light" href="{{route("forumPostByUsers", ['id'=>$user->id])}}">{{$user->name}} {{$user->first_name}}</a></li>
                            @endforeach
                ...
                <div class="mt-3 mb-2 cadreblanc">
                    <form action="{{route('forumPostByKeyWord')}}" method="post">
                        ...
                        <button type="submit" class="rounded-2 degrade-diagonal-rouge">Rechercher</button>
                    </form>
                </div>
            ...
            <div class="col-md-9 justify-content-center align-content-center w-100">
                @foreach ($categories as $category)
                    ...
                    <h5 id="theme{{$theme->id}}" class="d-inline-block font-weight-bold mt-2 text-with-shadow">{{$theme->label}}</h5>
                        @foreach ($theme->forum_post as $post)
                            <div class="card mb-1 degrade-diagonal-rouge text-white">
                                <div class="card-body p-1">
                                    {{--card enroulée--}}
                                    <div class="forum-titre" style="cursor: pointer;">
                                        <div class="d-flex">
                                            <div class="fw-bold me-2">{{$post->user->name}}</div>
                                            <div>{{$post->created_at->format('d/m/Y')}}</div>
                                            @auth
                                                @if(auth()->user()->isAdmin())
                                                    <a class="ms-auto p-0 m-0" href="{{route('showForumPost', ['id'=>$post->id])}}"><img src="{{ asset('images/editer.png') }}" alt="Editer"></a>
                                                @endif
                                            @endauth
                                        </div>
                                    ...
                                    {{--card deroulée--}}
                                    <div class="forum-content" style="display: none;">
                                        <div class="d-flex mb-4">
                                            <div class="ms-3">
                                                {{$post->content}}
                                                @foreach ($post->forum_responses as $repost)
                                                <div class="d-flex mt-4">
                                                    <div class="ms-3">
                                                        <div class="d-flex">
                                                            <div class="fw-bold me-2">{{$repost->user->name}}</div>
                                                            <div>{{$post->created_at->format('d/m/Y')}}</div>
                                                        </div>
                                            ...
                                            <form action="{{ route('storeForumResponse') }}" method="post" class="mb-4">
                                            @csrf
                                            <input type="hidden" name="forum_post_id" value="{{ $post->id }}">
                                            <textarea class="form-control" name="content" rows="3" placeholder="Rediger une reponse"></textarea>
                                            <button type="submit" class="rounded-2 degrade-diagonal-rouge float-sm-end mt-1">{{Auth::check()?'Soumettre':'Se connecter'}}</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
    ...
    {{--modal form--}}
    <div class="modal fade" id="modalFormForum" tabindex="-1" role="dialog" aria-labelledby="ModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content  degrade-diagonal">
                <div class="modal-header pb-1 pt-1">
                    <h5 class="modal-title text-with-shadow" id="ModalLabel text-with-shadow">ouvrir une discussion</h5>
                ...
                <div class="modal-body pb-1 pt-1">
                    <form action="{{ route('storeForumPost') }}" method="post">
                        @csrf
                        ...
                        <div class="mt-2">
                            <label for="content" class="text-with-shadow">Message</label>
                            <textarea class="form-control  degrade-diagonal-rouge text-light" id="content" name="content" placeholder="Contenu du message" rows="10" required>{{old('content')}}</textarea>
                        </div>

                        <button type="submit" class="rounded-2 degrade-diagonal-rouge float-end mt-1">Soumettre</button>
                    </form>
                </div>
            </div>
        </div>
...
<script>
    ...
    $('.forum-titre').on('click', function() {
        $('.forum-content').not($(this).next('.forum-content')).slideUp(); // Ferme les autres éléments
        $(this).next('.forum-content').slideToggle();
    });
});
</script>
