    ...
    <div class="row cadreblanc mt-1 mb-4">
        <div class="col-sm-10 d-flex justify-content-center align-items-center">
            ...
            <div class="col-sm-2 d-flex align-items-center ">
                <form id="deletePostForm" class="justify-content-center mb-1 mb-sm-0" action="{{route('deleteForumPost', ['id'=>$post->id])}}" method="post">
                    @csrf
                    @method('DELETE')
    ...
    {{--post principal--}}
        ...
        <div class="d-flex mb-4">
            <div class="ms-3 text-light">
                ...
                {{--liste des reponses--}}
                <div class="mt-4 p-2 cadreblanc">
                    {{--header des reponses--}}
                    <div class="row d-flex mb-2">
                        <div class="col-sm-9">
                            <div class="fw-bold me-2 text-light">{{$repost->user->name}}</div>
                            <div class="me-2 text-light">{{$repost->created_at->format('d/m/Y')}}</div>
                        </div>
                        <div class="col-sm-3">
                            <form id="deleteReponseForm" action="{{route("deletePostResponse", ['id'=>$repost->id])}}" method='post' class="ms-auto">
                            @csrf
                            @method('DELETE')
                                <button type="button" class="degrade-diagonal-danger rounded-2" onclick="confirmReponseDelete()">Supprimer la reponse</button>
                            </form>
                        </div>
                    </div>
                    {{--corps des reponses--}}
                    <div class="text-light">{{$repost->content}}</div>
                </div>
                @endforeach
            </div>
        </div>
...
<div class="modal fade" id="categoryModal" tabindex="-1" role="dialog" aria-labelledby="categoryModal" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form action="{{route('updateForumPost')}}" method="post">
            @csrf
...
{{--Ajout d'un pop up pour confirmer la suppression d'une reponse--}}
<script>
    function confirmReponseDelete() {
        if (confirm("Êtes-vous sûr de vouloir supprimer cette reponse ?")) {
            document.getElementById('deleteReponseForm').submit();
        }
    }
</script>
